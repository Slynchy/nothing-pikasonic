...
export default class OSUJSON {
    ...
    switch (sectionName) {
      ...
      case "Difficulty":
        ...
        while ((lineDifficulty = splitFile[++index]).trim() !== "") {
          ...
          lineDifficulty = lineDifficulty
            .substring(lineDifficulty.indexOf(":") + 1)
            .trim();
          section[key] = isNaN(parseFloat(lineDifficulty))
            ? lineDifficulty
            : +lineDifficulty;
        }
        return section;
  ...
  static _handleEvents(splitFile, index) {
    ...
    for (
      let i = index;
      i < splitFile.length && splitFile[i].trim() !== "";
      i++
    ) {
      let lineSplit = splitFile[i].trim().split(",");
      if (
        lineSplit.length === 1 ||
        (lineSplit[0][0] === "/" && lineSplit[0][1] === "/")
      )
        continue;
  ...
  /**
   * @fixme This function does not handle inherited MPB from multiple timing points
   * @private
   */
  ...
  static _getSampleSetType(sampleSet) {
    if (typeof sampleSet === "undefined")
      throw new Error("Undefined addition set!");
  ...
  static _handleSlider(splitLine, entryRef) {
    ...
    if (splitLine.length > 8) {
      ...
      for (let i = 0; i < entryRef["edgeHitsounds"].length; i++) {
        entryRef["edgeHitsounds"][i] = this._getHitSoundFromBitflag(
          entryRef["edgeHitsounds"][i],
        );
      }

      if (entryRef["edgeHitsounds"].length !== entryRef["repeat"] + 1) {
        throw new Error(
          "Number of edge hitsounds does not match repeat+1 syntax!",
        );
      }
    ...
    while ((currLine = splitFile[++index]).trim() !== "") {
      ...
      switch (entry.type.type) {
        ...
        case "new_combo": // ??
          break;
        case "osu!mania":
          entry["endTime"] = parseInt(
            currLineSplit[5].substring(0, currLineSplit[5].indexOf(":")),
          );
          break;
        default:
          entry["extras"] = this._handleExtras(
            currLineSplit[currLineSplit.length - 1],
          );
          break;
      }
   ...
   */
  static ParseOSUFileAsync(file): Promise<OsuFile> {
    return new Promise((resolve, reject) => {
      ...
      for (let i = 0; i < split.length; i++) {
        ...
        if (current.length < 2) continue; // Most likely an empty line
      ...
      output["RequiredFiles"] = OSUJSON._getRequiredSamples(
        output["TimingPoints"],
        output["HitObjects"],
      );

      resolve(output as OsuFile);
    });
  }
}